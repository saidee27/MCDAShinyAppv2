...
server <-  function(input, output, session){
 ...
 observeEvent(input$tabs, {
  ...
  res1 <- eventReactive(input$applym, {
      ...
      topind = which(c(input$criterion_max, input$criterion_min)==input$topalt)
      if(topind!=1) {
        c = mergeConstraints(lowerRatioConstraint(length(w),topind,1,100/(w[[1]][2])),upperRatioConstraint(length(w),topind,1,100/(w[[1]][1])))
      }
      
      for(j in 2:length(w)) { if(topind!=j) { c = mergeConstraints(c,lowerRatioConstraint(length(w),topind,j,100/(w[[j]][2])),upperRatioConstraint(length(w),topind,j,100/(w[[j]][1])) )} }
...
Begin by uplodaing the data and defining relevant fields using the 'Data Source' Item on the sidebar. Once uploaded you can view the Data on the 'Arranged Data' tab.", collapse = " "))
  ...
  output$report_br<- downloadHandler(
    ...
    content = function(file) {
      ...
      tempfile <- paste(tempfile(), ".Rmd")
      #tempReport <- file.path(tempdir(), "report_br.Rmd")
      file.copy("report_br.Rmd", tempfile, overwrite = TRUE)
      ...
      rmarkdown::render(tempfile, output_file = file,
                        params = params,
